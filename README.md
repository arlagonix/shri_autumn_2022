# Школа Разработки Интерфейсов. Осень 2022. Контест

<sub><strong>Сайт школы:</strong> https://academy.yandex.ru/schools/frontend</sub>

## Задачи

<table>
  <tr>
    <th>№</th>
    <th>Название</th>
    <th>Сложность</th>
    <th>Краткое описание</th>
    <th>Пройдено&nbsp;тестов&nbsp;/<br>Набрано баллов</th>
  </tr>
  
  <tr>
    <td>1</td>
    <td><a href="./1%20-%20%D0%A4%D0%BE%D1%82%D0%BE%D1%80%D0%B0%D0%BC%D0%BA%D0%B8">Фоторамки</a></td>
    <td align="center"><img src="https://img.shields.io/badge/-НИЗКАЯ-green?style=flat"></td>
    <td>
      <strong>Темы</strong>: Базовый JavaScript
      <br><br>
      <strong>На вход:</strong>
      <br>Число фоторамок
      <br>Ширина области для фоторамок
      <br>Высота области для фоторамок
      <br><br>
      <strong>На выход:</strong>
      <br>Функция: возвращает массив объектов с шириной, высотой, x, y каждой рамки
    </td>
    <td align="center"><code>17 / 20</code> ∙ <code>17</code></td>
  </tr>
  
  <tr>
    <td>2</td>
    <td><a href="./2%20-%20Список%20товаров">Список&nbsp;товаров</a></td>
    <td align="center"><img src="https://img.shields.io/badge/-НИЗКАЯ-green?style=flat"></td>
    <td>
      <strong>Темы</strong>: Верстка, HTML, CSS
      <br><br>
      Задача сверстать карточку с товарами максимально близко к макету из Figma
    </td>
    <td align="center"><code>1 / 1</code> ∙ <code>10</code></td>
  </tr>
  
  <tr>
    <td>3</td>
    <td><a href="./3%20-%20Figma%20to%20HTML">Figma&nbsp;to&nbsp;HTML</a></td>
    <td align="center"><img src="https://img.shields.io/badge/-СРЕДНЯЯ-yellow?style=flat"></td>
    <td>
      <strong>Темы</strong>: Обход дерева, HTML, разбор документации
      <br><br>
      <strong>На вход:</strong>
      <br>JSON-файл макета Figma (выгружен из их API)
      <br><br>
      <strong>На выход:</strong>
      <br>Функция: преобразует этот JSON в HTML<br>(стили тоже нужно перенести)
      <br><br>
      <em>(решил, но не прошло тесты)</em>
    </td>
    <td align="center"><code>0 / 2</code> ∙ <code>0</code></td>
  </tr>
  
  <tr>
    <td>4</td>
    <td><a href="./4%20-%20Лабиринт">Лабиринт</a></td>
    <td align="center"><img src="https://img.shields.io/badge/-СРЕДНЯЯ-yellow?style=flat"></td>
    <td>
      <strong>Темы</strong>: Promise
      <br><br>
      <strong>На вход:</strong>
      <br>Начальная точка в лабиринте
      <br>Объект для взаимодействия с лабиринтом
      <br><br>
      <strong>На выход:</strong>
      <br>Функция: возвращает точку x,y выхода из лабиринта
      <br><br>
      <em>(не решал)</em>
    </td>
    <td align="center"><code>0 / 8</code> ∙ <code>0</code></td>
  </tr>
  
  <tr>
    <td>5</td>
    <td><a href="./5%20-%20Объявление">Объявление</a></td>
    <td align="center"><img src="https://img.shields.io/badge/-СРЕДНЯЯ-yellow?style=flat"></td>
    <td>
      <strong>Темы</strong>: CSS, JS, DOM, расчет сетки
      <br><br>
      <strong>На вход:</strong>
      <br>Элемент в котором находятся объявления
      <br>Расстояние между колонками, а также элементами по-вертикали
      <br><br>
      <strong>На выход:</strong>
      <br>Функция: располагает DOM-элементы объявлений так, чтобы они раместились в указанном числе колонок с указанным расстоянием между ними
      <br><br>
      <em>(решил, но не прошло тесты)</em>
    </td>
    <td align="center"><code>0 / 8</code> ∙ <code>0</code></td>
  </tr>
  
  <tr>
    <td>6</td>
    <td><a href="./6%20-%20Drag%20and%20Drop">Drag&nbsp;and&nbsp;Drop</a></td>
    <td align="center"><img src="https://img.shields.io/badge/-СРЕДНЯЯ-yellow?style=flat"></td>
    <td>
      <strong>Темы</strong>: «Оживление» HTML и CSS с помощью JS, работа с DOM-событиями
      <br><br>
      Задача реализовать возможность добавлять, удалять теги с писем методом Drag&Drop
    </td>
    <td align="center"><code>10 / 12</code> ∙ <code>25</code></td>
  </tr>
  
  <tr>
    <td>7</td>
    <td><a href="./7%20-%20Многопоточные%20вычисления">Многопоточные вычисления</a></td>
    <td align="center"><img src="https://img.shields.io/badge/-ВЫСОКАЯ-red?style=flat"></td>
    <td>
      <strong>Темы</strong>: Promise, структуры данных
      <br><br>
      <strong>На вход:</strong>
      <br>Массив строк с данными, которые могут быть произвольными
      <br><br>
      <strong>На выход:</strong>
      <br>Promise, который резолвит результат строкой
      <br><br>
      <em>(не решал)</em>
    </td>
    <td align="center"><code>0 / 15</code> ∙ <code>0</code></td>
  </tr>
  
  <tr>
    <td>8</td>
    <td><a href="./8%20-%20Применить%20стили">Применить&nbsp;стили</a></td>
    <td align="center"><img src="https://img.shields.io/badge/-ВЫСОКАЯ-red?style=flat"></td>
    <td>
      <strong>Темы</strong>: CSS, JS, обход дерева
      <br><br>
      <strong>На вход:</strong>
      <br>Набор CSS-правил и HTML-дерево
      <br><br>
      <strong>На выход:</strong>
      <br>Функция: применяет CSS (через styles), выдаёт итоговый HTML
      <br><br>
      <em>(не решал)</em>
    </td>
    <td align="center"><code>0 / 6</code> ∙ <code>0</code></td>
  </tr>
   
</table>

## Про доступ к контесту

Чтобы получить доступ к задачам, нужно заполнить анкету. Ссылка на неё - на официальном сайте школы: https://academy.yandex.ru/schools/frontend

Далее на почту придёт письмо с ссылками на задания:

<p align="center"><img src="./images/letter.jpg" width="100%"></p>

## Про задачи

Необязательно решать все задачи. Можно выбрать только те задания, которые ближе по уровню и теме.
 
Отдельные задачи можно выполнять постепенно, в любом порядке и в удобное время. Но в рамках каждой задачи есть ограничения:
* На решение дано 3 часа; прерываться и ставить на паузу — нельзя. 
* Отсчёт начнётся после нажатия на кнопку «Стартовать».
* У вас будет только одна попытка, поэтому проверьте стабильность интернет-соединения.

Решения можно отправлять только до 14 сентября 2022 г., 23:59 по московскому времени. 
Если хорошо справиться с заданиями, Yandex пригласит вас на собеседование.

На собеседовании (по словам руководителей школы) дадут алгоритмическую задачку, чтобы убедиться, что человек сам всё решал, и поговорят о жизни. Собеседование не более, чем на 30 мин

## Дополнительно

Тут руководители школ отвечают на вопросы по обучению и не только (сентябрь 2022): https://www.youtube.com/watch?v=h6WO2vY6NE4

# Итог отбора

Меня не взяли. Вот официальный ответ:
<em>Здравствуйте!
Спасибо за интерес к Школе разработки интерфейсов и уделённое нам время.

Мы внимательно изучили вашу анкету и тестовое задание. К сожалению, мы пока не готовы предложить вам участие в Школе. Но будем рады, если вы решите попробовать свои силы в следующий раз.

Чтобы не пропустить старт отбора на следующий поток школ Академии Яндекса, подписывайтесь на наши социальные сети. Именно там мы публикуем полезную и актуальную информацию: списки книг и курсов, интервью с участниками предыдущих Школ, дайджест IT-мероприятий, вакансии в проектах Яндекса и другое. Туда же мы будем выкладывать карточки и материалы от преподавателей.

Присоединяйтесь — будет интересно!

До встречи!</em>

Странно, что в письме не указали, в чем именно причина отказа. То ли баллов мало, то ли ещё что.

Обидно, конечно, что не взяли. Столько времени потратил на задания, немало решил. Неужели нашлось 80-100 человек, которые решили задания лучше меня? Стоило ли столько времени тратить на то, чтобы поступить туда? Не знаю. Тем не менее я не особо расстроен, продолжу дальше самостоятельно изучать фронтенд
